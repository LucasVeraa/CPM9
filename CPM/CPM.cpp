...
int CPM::Matching(FImage& img1, FImage& img2, FImage& outMatches)
{
	...
	t.tic();
	vector<int> cellSizeVect{2, 4, 8};

	for (int i = 0; i < nLevels; i++){
		imDaisy(_pyd1[i], _im1f[i]);
		imDaisy(_pyd2[i], _im2f[i]);
 		//ImageFeature::imSIFT(_pyd1[i], _im1f[i], cellSizeVect, 1, true, 8);
 		//ImageFeature::imSIFT(_pyd2[i], _im2f[i], cellSizeVect, 1, true, 8);
 		//ImageFeature::imSIFT(_pyd1[i], _im1f[i], 8, 1, true, 8);
 		//ImageFeature::imSIFT(_pyd2[i], _im2f[i], 8, 1, true, 8);
	}
	//t.tic();
	int step = _step;
	int gridw = w / step;
...
void CPM::OnePass(FImagePyramid& pyd1, FImagePyramid& pyd2, UCImage* im1f, UCImage* im2f, IntImage& seeds, IntImage& neighbors, FImage* pydSeedsFlow)
{
	...
	real_time = (t4.tv_sec - t3.tv_sec) + (float) (t4.tv_usec - t3.tv_usec) / 1000000;
	//printf("OnePass 2: %f \n", real_time);
	...
	gettimeofday(&t6, 0);
	real_time = (t6.tv_sec - t5.tv_sec) + (float) (t6.tv_usec - t5.tv_usec) / 1000000;
	//printf("OnePass 3: %f \n", real_time);
